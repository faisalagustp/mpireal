...
*******************************************************************
                  ...
                  Example 23 (mm_mult_fox.c)

  Objective           : Matrix Matrix multiplication
                        (Using Cartesian Topology, FOX Algorithm)
...
int main (int argc, char *argv[])
{
  ...
  int Matrix_Size[4];
  int source, destination, send_tag, recv_tag, Bcast_root, numtasks;
  double    start, end, startcomm, endcomm, commtime, totalcomm;


  float **Matrix_A, **Matrix_B, **Matrix_C;
  float *A_Bloc_Matrix, *B_Bloc_Matrix, *C_Bloc_Matrix, *Buffer;
  ...
  Buffer = (float *) malloc(A_Bloc_MatrixSize*sizeof(float));
  ...
  for(istage=0; istage<grid.p_proc; istage++){
	  Bcast_root = (grid.Row + istage) % grid.p_proc;
	  if(Bcast_root == grid.Col){
	     MPI_Bcast(A_Bloc_Matrix, A_Bloc_MatrixSize, MPI_FLOAT, Bcast_root, grid.Row_comm);
		  index=0;
		  for(irow=0; irow<NoofRows_BlocA; irow++){
		      for(icol=0; icol<NoofCols_BlocB; icol++){
	    ...
	    MPI_Bcast(Buffer, A_Bloc_MatrixSize, MPI_FLOAT, Bcast_root, grid.Row_comm);
		  index=0;
		  for(irow=0; irow<NoofRows_BlocA; irow++){
		      for(icol=0; icol<NoofCols_BlocB; icol++){
			  for(jcol=0; jcol<NoofCols_BlocA; jcol++){
				C_Bloc_Matrix[index] += Buffer[irow*NoofCols_BlocA + jcol] *
				         B_Bloc_Matrix[jcol*NoofCols_BlocB + icol];
				  }
			     index++;
			  }
     ...
     MPI_Sendrecv_replace(B_Bloc_Matrix, B_Bloc_MatrixSize, MPI_FLOAT, destination,
			  send_tag, source, recv_tag, grid.Col_comm, &status);
  ...
  MPI_Gather (C_Bloc_Matrix, NoofRows_BlocA * NoofCols_BlocB, MPI_FLOAT, MatC_array,
	           NoofRows_BlocA * NoofCols_BlocB, MPI_FLOAT, Root, grid.Comm);
